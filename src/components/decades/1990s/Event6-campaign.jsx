import "./style-1990.css";

const Event6 = () => {
  return (
    <div className="event-container narrative panel-yellow">
      <div>
        <h1 className="date">December 5</h1>
        <h1 className="year">1991</h1>
        <h2>Growing bigger</h2>
        <p>
          The Board approved an enrollment increase to 1,000 students and began
          the planning process for a major fundraising campaign. Launched to
          coincide with the College’s 50th anniversary in 1996, “The Campaign
          for CMC: Fifty Years of Leadership.”
        </p>
      </div>
    </div>
  );
};

export default Event6;
